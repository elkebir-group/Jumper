...
class solveTranscription():
    ...
    def __init__(self, graph=None, numPaths=1, verbose=True, ref=None,
        ...
        if samplingFrequency <= 1:
            raise Exception(
                f"sampling frequecny {samplingFrequency} too low for solveTranscription")
        else:
    ...
    def solve_greedy_likelihood(self):
        ...
        lmin = solveTranscription.find_shortest_transcript_length(
            spliceEdges, spliceLengths, self.ref_length)
        self.chosen_paths = []

        if self.verbose:
            print('-'*50)
            print(
                f"max phasing count is {max([count for _, _, count in self.graph.phasing])}")
            print(
                f"total reads is {sum([count for _, _, count in self.graph.phasing])}")
            print('-'*50)
        ...
        obj_scaling_factor = 10**4 / \
            max([count for _, _, count in self.graph.phasing]) / abs(F[0])
        ...
        while iter_count <= self.max_iter or flag in [1, 2, 3]:
            ...
            g = model.addVars(npaths, lb=0, ub=1,
                              vtype=gp.GRB.CONTINUOUS, name='g')
            ...
            for idx in range(self.graph.numSpliceEdges):
                for pid in range(npaths):
                    ...
                    model.addConstr(ysplice[idx, pid] >=
                                    g[pid] + w[idx, pid] - 1)
            ...
            for idx in range(self.graph.numPhasing):
                ...
                for pathIndex, path in enumerate(self.chosen_paths):
                    pathSpliceEdges = [
                        edge for edge in path if edge.type == 'splice']
            ...
            if model.status == gp.GRB.Status.OPTIMAL or model.status == gp.GRB.Status.TIME_LIMIT:
                ...
                if flag == 1:
                    ...
                    npaths = 0
                    npaths_added = self.add_path_to_chosen_with_expansion(
                        solW, spliceEdges)
                    if npaths_added == 0:
                        if self.verbose:
                            print('+'*50)
    ...
    def choose_best_k_paths(self, solC, kdx):
        ...
        chosen_indices = [i for i, v in sorted(solC.items(), key=lambda x: (-x[1] * self.graph.getPathLength(
            self.chosen_paths[x[0]]), self.graph.getPathIndex(self.chosen_paths[x[0]])))][:kdx]
    ...
    def expand_all_paths(self):
        ...
        for path in original_paths:
            ...
            for case in range(2**npath_spliceEdges - 1):
                expansionIndex = bin(case).replace(
                    "0b", "").zfill(npath_spliceEdges)
    ...
    def solve_likelihood(self):
        ...
        lmin = solveTranscription.find_shortest_transcript_length(
            spliceEdges, spliceLengths)
        lend = self.ref_length

        g = model.addVars(self.numPaths, lb=0, ub=1,
                          vtype=gp.GRB.CONTINUOUS, name="g")
        ...
        for idx in range(self.graph.numPhasing):
            for pathIndex in range(self.numPaths):
                ...
                for edge in splus:
                    # spliceIndex = spliceEdges.index(self.graph.edges[edgeIndex])
                    spliceIndex = spliceEdge2Index[edge]
                    model.addConstr(y[idx, pathIndex] <=
                                    w[spliceIndex, pathIndex])
                    expr_plus += w[spliceIndex, pathIndex]

                for edge in sminus:
                    # spliceIndex = spliceEdges.index(self.graph.edges[edgeIndex])
                    spliceIndex = spliceEdge2Index[edge]
                    model.addConstr(y[idx, pathIndex] <= 1 -
                                    w[spliceIndex, pathIndex])
        ...
        if self.verbose:
            ...
            print(f"number of splice edges is {self.graph.numSpliceEdges}")
            print(
                f"number of transcript splice edges is {numTranscriptSpliceEdges}")
            print(f"number of phasing reads is {self.graph.numPhasing}")
            print('-'*50)
    ...
    def decodeContinuousSolutionLikelihood(self, solW, solWeight, spliceEdges=None):
        ...
        for pathIndex in range(self.numPaths):
            ...
            solPaths.append(
                (path_id, path, solWeight[pathIndex] / total_abundance))
    ...
    def filterSolution(self, threshold, read_length=100):
        ...
        if self.verbose:
            print(
                f"filtered out {prefilter_number - len(self.solution)} transcripts")
        ...
        with open(filename, 'w') as output:
            if self.transcriptWeights:
                for idx, weight in self.transcriptWeights:
                    output.write(
                        f">transcript_{idx}\t{self.graph.transcript_type[idx]}:{weight}\n")
            ...
            if self.transcriptWeights:
                for idx, weight in self.transcriptWeights:
                    ...
                    if len(spliceEdges) == 0:
                        output.write(
                            f"{contig}\tjumper\ttranscript\t{path[0].left.left}\t{path[-1].right.right}\t1000\t.\t.\tRPKM \"{weight}\"\n")
                    else:
                        output.write(
                            f"{contig}\tjumper\ttranscript\t{path[0].left.left}\t{path[-1].right.right}\t1000\t.\t.\tRPKM \"{weight}\"\n")
                        output.write(
                            f"{contig}\tjumper\texon\t{path[0].left.left}\t{spliceEdges[0].left.right}\t1000\t.\t.\t.\n")
                        output.write(
                            f"{contig}\tjumper\texon\t{spliceEdges[0].right.left}\t{path[-1].right.right}\t1000\t.\t.\t.\n")
                ...
                for pathIndex, path, weight in self.solution:
                    ...
                    if len(spliceEdges) == 0:
                        ...
                        for idx, edge in enumerate(spliceEdges):
                            if idx == 0:
                                output.write(
                                    f"{contig}\tjumper\texon\t{path[0].left.left}\t{edge.left.right}\t1000\t.\t.\t.\n")
                            else:
                                output.write(
                                    f"{contig}\tjumper\texon\t{spliceEdges[idx-1].right.left}\t{edge.left.right}\t1000\t.\t.\t.\n")
                        output.write(
                            f"{contig}\tjumper\texon\t{edge.right.left}\t{path[-1].right.right}\t1000\t.\t.\t.\n")
