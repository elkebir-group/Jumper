...
def chooseFromProb(probs, cum = False):
    ...
    for idx in range(len(cumprobs)):
        if rand_num <= cumprobs[idx]:
            return idx
    raise Exception(f"rand_num ({rand_num}) more than cumprobs[-1] ({cumprobs[-1]})")
...
def getTerminalNodes(fragment, path_nodes):
    ...
    if not left_node:
        raise Exception(f"left node not found for fragment ({fragment[0]}, {fragment[1]})")
        
    if not right_node:
        raise Exception(f"right node not found for fragment ({fragment[0]}, {fragment[1]})")
...
class simulator():
    ...
    def generatePhasingReads(self):
        ...
        for idx, path in enumerate(self.paths):
            for edge in path:
                if edge.type == 'splice':
                    self.path_lengths[idx] -= (edge.right.left - edge.left.right)
        
        probs = [self.abundances[idx] * self.path_lengths[idx] / self.npaths for idx in range(numPaths)]
        ...
        for idx in range(self.nreads):
            ...
            self.total_log_prob_reads += math.log(probs[pathIndex] / sum(probs) / self.path_lengths[pathIndex])
            ...
            read1_start = np.random.random() * (curr_length - 1 - 2 * self.readlength - self.insertlength) + 1
            read2_start = read1_start + self.readlength + self.insertlength

            # print(f"read start locations are {read1_start}, {read2_start}")
    
            curr_path_nodes = [curr_path[0].left] + [edge.right for edge in curr_path]
            ...
            splus_tuples = [(edge.left.right, edge.right.left) for edge in splus]
            sminus_tuples = [(edge.left.right, edge.right.left) for edge in sminus]
    
            phasing_reads[tuple([tuple(sorted(splus_tuples)),tuple(sorted(sminus_tuples))])] += 1
    ...
    def writeReadCounts(self, fname):
        ...
        with open(fname, 'w') as output:
            for _, val in pathTuples:
                output.write(f"{math.floor(self.nreads * val / totalEffectiveLength)}\n")
                #output.write(f"{val}\n")
...
def main(args):
    ...
    if args.verbose:
        ...
        print(f"abudances are -- {[x / instance.npaths for x in instance.abundances]}")
        print(f"total log probability paths is -- {instance.total_log_prob_path}")
        print(f"total log probability reads is -- {instance.total_log_prob_reads}")
        print(f"sense is {args.sense}")
    
    if args.outputPaths:
        instance.graph.writePaths(instance.paths, args.outputPaths, instance.abundances)
...
def get_options():
    ...
    parser.add_argument("--outputCanonical", type = str, help="output file for transcripts")    
    ...
    parser.add_argument("--npaths", type=int, help = 'number of paths to draw [1]', default = 1)
    parser.add_argument("--outputPaths", type=str, help="output file randomly chosen paths")
    parser.add_argument("--sense", default=True, const=True, type=str2bool, nargs='?', help="set pos strand flag for transcript generation")
    parser.add_argument("--nreads", type=int, help="number of reads to be generated [100]", default = 100)
    parser.add_argument("--outputPhasing", type=str, help="output file for phasing reads")
    parser.add_argument("--readlength", type=int, help="length of the reads (one end)", default = 100)
    parser.add_argument("--insertlength", type=int, help="length of the gap between the paired ends", default = 50)
    parser.add_argument("--canonicalprior", type=int, help="alpha value canonical discontinuous edge", default = 10)
    parser.add_argument("--seed", type=int, help="seed for the random number generators", default = 0)
    parser.add_argument("--outputFasta", type=str, help="output fasta file for drawn paths")
    parser.add_argument("--outputReadCounts", type=str, help="output file with number of reads drawn per transcript")
    parser.add_argument("--canonical", type=str2bool, help="flag to output only canonical transcripts [False]", default=False)
