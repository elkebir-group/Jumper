...
class myedge():
    ...
    def __init__(self, left_node=None, right_node=None, weight=None, name=None, edgetype=None):
        ...
        if left_node is None and right_node is None:
            ...
            assert(left_node.right <=
                   right_node.left), "nodes must be ordered [myedge()]"
            self.left = left_node
            ...
            raise Exception(
                "either both nodes must be defined or neither! [myedge()]")
...
class segmentGraph():
    ...
    def __init__(self, breakpoints=None, edges=None, phasing=None, ref=None, verbose=True, edge_threshold=0, phasing_threshold=0, width=0, threads=1):
        ...
        if len(breakpoints) > 0:
            self.nodes.append(
                mynode(right=breakpoints[0], length=ref.lengths[0]))
            ...
            if len(breakpoints) > 1:
                self.nodes.append(
                    mynode(left=breakpoints[-1], length=ref.lengths[0]))
        ...
        self.numSpliceEdges = len(
            [edge for edge in self.edges if edge.type == 'splice'])
    ...
    def getPhasing(self) -> List[Tuple[int, int, str]]:
        ...
        for phase in self.phasing:
            ...
            for edge in phase:
                curr_phase.append(
                    (edge.left.right, edge.right.left, edge.type))
    ...
    def __sub__(self, other: 'segmentGraph') -> 'segmentGraph':
        ...
        keyList = [(edge.left.right, edge.right.left, edge.type)
                   for edge in self.edges]
    ...
    def writeGraph(self, fname) -> None:
        ...
        with open(fname, 'w') as output:
            ...
            for edge in self.edges:
                output.write(
                    f"{edge.left.name}\t{edge.right.name}\t{edge.type}\t{edge.weight}\n")
    ...
    def writeDOT(self, fname) -> None:
        ...
        with open(fname, 'w') as output:
            ...
            for node in self.nodes:
                ...
                output.write(
                    f"\t{idx} [label=\"{node.name}\", style=\"bold\"];\n")
                idx += 1
            ...
            for edge in self.edges:
                ...
                # if edge.weight >0 :
                output.write(
                    f"\t{left_idx} -> {right_idx} [label=\"{edge.weight}\", fillcolor={edgeColor[edge.type]}, style=\"bold\"];\n")
                # else:
    ...
    def stitchPath(self, path_spliceEdges):
        ...
        for edge in path_spliceEdges:
            ...
            if len(subpath) > 1:
                raise Exception(
                    f"more than 1 canonical subpath for {left_id} -> {right_id}")
            if len(subpath) == 1:
    ...
    def writePaths(self, paths, fname, info=[None]):
        if all(info):
            ...
            pathTuples = [(self.getPathIndex(path), path, info[idx])
            ...
            for element in pathTuples:
                ...
                if all(info):
                    if pathIndex <= self.numTranscripts:
                        output.write(
                            f">transcript_{pathIndex}: {element[2]}\n")
                    else:
                ...
                for edge in path:
                    output.write(
                        f"{edge.left.name}\t{edge.right.name}\t{edge.type}\n")
    ...
    def writeFasta(self, paths, fname, info=[None]):
        ...
        with open(fname, 'w') as output:
            for element in pathTuples:
                ...
                for edge in path:
                    if edge.left.left == 1:
                        genome += self.ref.fetch(
                            self.ref.references[0], 0, edge.left.right)
                    else:
                        genome += self.ref.fetch(
                            self.ref.references[0], edge.left.left, edge.left.right)
                genome += self.ref.fetch(
                    self.ref.references[0], path[-1].right.left, path[-1].right.right)
    ...
    def getCanonicalPaths(self, startIndex, endIndex):
        ...
        if startIndex > endIndex:
            raise Exception(
                f"start index {startIndex} must be less than or equal to index {endIndex}!")
    ...
    def initCanonicalTranscripts(self, ref=None):
        ...
        if not ref:
            if not self.ref:
                raise Exception(
                    'cannot initialize canonical transcripts without reference!')
            else:
                ref = self.ref
        else:
            self.ref = ref
        ...
        for idx, edge in enumerate(self.transcriptSpliceEdges):
            ...
            rightpaths = self.getCanonicalPaths(
                _3primeIndex, self.numNodes - 1)
        ...
        print(
            f"number of transcript splice edges is {len(self.transcriptSpliceEdges)}")
        print('-'*50)

    def coarsenEdgeWeights(self, resolution):
        for edge in self.edges:
            edge.weight = int(math.floor(
                float(edge.weight) / float(resolution)))
    ...
    def writeTranscripts(self, fname):
        with open(fname, 'w') as output:
            for idx, transcript in enumerate(self.transcripts):
                output.write(
                    f">transcript_{idx}\t{self.transcript_type[idx]}\n")
    ...
    def prune_edges(self):
        self.edges = [edge for edge in self.edges if edge.weight >=
                      self.edge_threshold or edge.type != 'splice']
...
def main(args):
    ...
    if args.inputPhasing:
        ...
        phasing_reads = SJcounter.get_phasing_reads(
            args.bam, splice_edges.keys(), args.width, args.paired)
...
def get_options():
    ...
    parser.add_argument("-k", type=int, dest='numPaths',
                        help="number of paths for the flow decomposition", default=1)
    ...
    parser.add_argument("--outputTranscripts", type=str,
                        help="output file for transcripts")
    parser.add_argument("--outputBreakpoints", type=str,
                        help="output file containing breakpoints")
    parser.add_argument("--outputEdges", type=str,
                        help="output file containing graph edges")
    parser.add_argument("--outputDecomposition", type=str,
                        help="output file for the decomposed non-canonical transcripts")
    parser.add_argument("--outputMatching", type=str,
                        help="output file for the matching of phasing reads to inferred transcripts")
    parser.add_argument("--outputGTF", type=str,
                        help="output file in GTF format")
    parser.add_argument("--report", type=str,
                        help="output file for report on the splice graph")
    ...
    parser.add_argument("--threads", type=int, default=1,
                        help='number of threads allowed to be used [1]')
    parser.set_defaults(verbose=True)
    parser.add_argument("--timelimit", type=int,
                        help="time limt for the gurobi solvers in seconds [None]", default=None)
    parser.add_argument("--maxIter", type=int,
                        help="maximum iterations for the greedy algorithm [100]", default=100)
