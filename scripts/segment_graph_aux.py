    ...
    with open(inputPhasing, 'r') as input:
        for line in input:
            if line.startswith('>'):
                ...
                data = line[1:-1].split(',')
                phase.append((int(data[0]), int(data[1]), data[2].split('\'')[1]))
        phasing.append(phase)
...
def generateGraphInput(SJcounter, bam, df_sj_reads, phasing, mutList, df_depth, ref, width, sj_threshold):
    ...
    for _, row in df_sj_reads.iterrows():
        start_dist = [abs(x + float(width)/2 - row['5prime']) for x in list(start_junction_clusters.keys())]
        min_start = min(start_dist)
        
        
        end_dist = [abs(x  + float(width)/2 - row['3prime']) for x in list(end_junction_clusters.keys())] 
        min_end = min(end_dist)

        if min_start <= float(width)/2 and min_end <= float(width)/2:
            start_pos = list(start_junction_clusters.keys())[start_dist.index(min_start)]
            end_pos = list(end_junction_clusters.keys())[end_dist.index(min_end)]
    ...
    for phase in phasing:
        ...
        for left_pos, right_pos, edgeType in phase:
            if edgeType == 'splice':
                left_dist = [abs(x - left_pos) for x in list(start_junction_clusters.keys())]
                min_left_dist = min(left_dist)
                
                right_dist = [abs(x - right_pos) for x in list(end_junction_clusters.keys())]
                ...
                if min_left_dist > width or min_right_dist > width:
                    ...
                    newphase.append((list(start_junction_clusters.keys())[left_dist.index(min_left_dist)],
                                     list(end_junction_clusters.keys())[right_dist.index(min_right_dist)],
                                     edgeType))
            else:
            ...
            for line in input:
                ...
                if edgetype == 'splice':
                    ...
                    if (left_pos, right_pos) not in adjacency_edges.keys():
                        adjacency_edges[(left_pos, right_pos)] = {edgetype: count}
                    else:
                        adjacency_edges[(left_pos, right_pos)][edgetype] = count
            ...
            if line.startswith('>'):
                if not flag:
                    ...
                    if tuple([tuple(splus),tuple(sminus)]) in phasing.keys():
                        print(f"phasing repeat -- {idx} -- {list(phasing.keys()).index(tuple([tuple(splus),tuple(sminus)]))}")
                        print(f"{splus}")
                        print(f"{sminus}")
        ...
        for edges in edgeList:
            for (left_pos, right_pos), edge_dict in edges.items():
                for edgeType, count in edge_dict.items():
                    output.write(f"{left_pos}\t{right_pos}\t{edgeType}\t{count}\n")
